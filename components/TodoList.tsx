...
import { 
  ...
  Clock, CalendarDays, Type, History, Recycle, Trash2, Undo2, ChevronRight, ChevronDown, Folder, CalendarRange
} from 'lucide-react';
...
export const TodoList: React.FC = () => {
  const { todos, selectedDate, viewMode, selectedCategoryId, categories, sortBy, sortDirection, upcomingDays, setSortBy, setSortDirection, updateCategory, updateTodo, restoreCategory, restoreTodo, permanentlyDeleteCategory, permanentlyDeleteTodo } = useTodoStore();
        ...
        {viewMode === 'trash' ? (
            trashRootCategories.length === 0 && trashRootTodos.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-gray-400 pb-20"><div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4"><Recycle size={24} className="text-gray-300" /></div><p className="text-sm font-medium">回收站是空的。</p></div>) : (
                <div className="space-y-1">
                    ...
                    {trashRootTodos.map(todo => (<div key={todo.id} className="group flex items-center justify-between py-2 px-3 rounded-md hover:bg-white border border-transparent hover:border-gray-100 hover:shadow-sm transition-all mb-1"><div className="flex items-center gap-3 overflow-hidden"><div className={`w-1.5 h-1.5 rounded-full ${todo.completed ? 'bg-green-400' : 'bg-gray-300'}`}></div><span className={`text-sm truncate ${todo.completed ? 'text-gray-400 line-through' : 'text-gray-600'}`}>{todo.title}</span>{todo.categoryId && categories.find(c => c.id === todo.categoryId) && (<span className="text-xs text-gray-400 px-1.5 py-0.5 bg-gray-100 rounded">{categories.find(c => c.id === todo.categoryId)?.name}</span>)}</div>
                    {/* Reverted Opacity */}
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={() => handleSmartRestoreTodo(todo)} className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="还原"><Undo2 size={16} /></button><button onClick={() => setPermDeleteTodoId(todo.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="永久删除"><Trash2 size={16} /></button></div></div>))}
                </div>
            )
        ) : (
            filteredTodos.length > 0 ? (filteredTodos.map(todo => (<TodoItem key={todo.id} todo={todo} />))) : (<div className="h-full flex flex-col items-center justify-center text-gray-400 pb-20"><div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4"><ListTodo size={24} className="text-gray-300" /></div><p className="text-sm font-medium">列表为空。</p><Button variant="ghost" size="sm" onClick={() => setIsModalOpen(true)} className="mt-2 text-primary-600">创建第一个任务</Button></div>)
        )}
      </div>
